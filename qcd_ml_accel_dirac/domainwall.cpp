...
at::Tensor domainwall_call_cpu (const at::Tensor& U, const at::Tensor& v, double mass, double m5){
    ...
    double prf0 [4] = {mass,mass,-1,-1};
    double prfN [4] = {-1,-1,mass,mass};

    // we split the values r==0 and r==v_size[0]-1 from the loop, as they are treated differently

    int64_t r = 0;
    // part of the loop for r==0
    // parallelisation of x
    at::parallel_for(0, v_size[1], 1, [&](int64_t start, int64_t end){
    for (int64_t x = start; x < start; x++){
                    ...
                    for (int64_t g = 0; g < 3; g++){
                        for (int64_t s = 0; s < 4; s++){
                            res_ptr[ptridx7(r,x,y,z,t,s,g,vstride)] =
                                ...
                                // domain wall term
                                +prf0[s]*v_ptr[ptridx7((r+prs[s]) %v_size[0],x,y,z,t,s,g,vstride)];
                        }
                    }
    ...
    // parallelisation of the center of the r loop
    at::parallel_for(1, v_size[0]-1, 1, [&](int64_t start, int64_t end){
                        ...
                        for (int64_t g = 0; g < 3; g++){
                            for (int64_t s = 0; s < 4; s++){
                                res_ptr[ptridx7(r,x,y,z,t,s,g,vstride)] =
                                    ...
                                    (5.0 - m5) * v_ptr[ptridx7(r,x,y,z,t,s,g,vstride)]
                                    // domain wall term
                                    -v_ptr[ptridx7((r+prs[s]) %v_size[0],x,y,z,t,s,g,vstride)];
                            }
                        }
    ...
    r = v_size[0]-1;
    // part of the loop for r==v_size[0]-1
    // parallelisation of x
    at::parallel_for(0, v_size[1], 1, [&](int64_t start, int64_t end){
                    ...
                    for (int64_t g = 0; g < 3; g++){
                        for (int64_t s = 0; s < 4; s++){
                            res_ptr[ptridx7(r,x,y,z,t,s,g,vstride)] =
                                ...
                                (5.0 - m5) * v_ptr[ptridx7(r,x,y,z,t,s,g,vstride)]
                                // domain wall term
                                +prfN[s]*v_ptr[ptridx7((r+prs[s]) %v_size[0],x,y,z,t,s,g,vstride)];
                        }
                    }
